Shader "Custom Lighting/Specular Lighting"
{
    Properties
    {
        _SpecIntensity    ("Specular Intensity", Float) = 1.0
        _SpecPower        ("Specular Power",     Range(4,256)) = 32
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // SRP Batcher-friendly material params
            ...
            // Classic Blinn–Phong (per-light)
            static half3 BlinnPhong(half3 N, half3 V, Light L, half specPower, half specIntensity)
            {
                // Half-vector between view and light
                half3 H  = normalize(L.direction + V);
                half  nh = saturate(dot(N, H));
                half  s  = pow(nh, specPower);

                return (s * specIntensity) * L.color;
            }
            ...
            half4 frag (Varyings IN) : SV_Target
            {
                ...
                half3 specular  = BlinnPhong(N, V, mainLight, _SpecPower, _SpecIntensity);
                ...
                // --- Additional lights
                    ...
                    for (uint li = 0; li < min(URP_FP_DIRECTIONAL_LIGHTS_COUNT, MAX_VISIBLE_LIGHTS); li++)
                    {
                        ...
                        diffuse += Lambert(N, L);
                        specular += BlinnPhong(N, V, L, _SpecPower, _SpecIntensity);
                    }
                    #endif
