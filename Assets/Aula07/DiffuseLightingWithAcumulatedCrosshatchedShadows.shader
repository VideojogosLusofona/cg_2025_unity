Shader "Custom Lighting/Diffuse Lighting (Crosshatched Accumulated Shadows)"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            static void Lambert(half3 N, Light L, half3 worldPos, inout half4 currentLight)
            {
                half nl = saturate(dot(N, L.direction));
                
                currentLight.xyz += nl * L.color * L.distanceAttenuation;
                currentLight.w *= L.shadowAttenuation;
            }
            ...
            half4 frag (Varyings IN) : SV_Target
            {
                ...
                half4 diffuse = half4(0, 0, 0, 1);
                Lambert(N, mainLight, IN.positionWS, diffuse);
                ...
                // --- Additional lights
                    ...
                    for (uint li = 0; li < min(URP_FP_DIRECTIONAL_LIGHTS_COUNT, MAX_VISIBLE_LIGHTS); li++)
                    {
                        ...
                        Lambert(N, L, IN.positionWS, diffuse);
                    }
                    #endif
                ...
                float2 uv = GetProjectedUV(IN.positionWS * _Scale, N);

                float hatchSample = SAMPLE_TEXTURE2D(_HatchTex, sampler_HatchTex, uv).r;
                float hatchMask = step(1 - diffuse.w, hatchSample);
                
                return half4(diffuse.xyz * hatchMask, 1);
            }
            ENDHLSL
        }
